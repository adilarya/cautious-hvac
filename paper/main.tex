...
\label{eq:diffusion}
\end{equation}
where $\nabla^2 T$ is the 6-neighbour Laplacian and
$S_{i,j,k}^{(t)}$ collects three source terms:
\begin{enumerate}
\itemsep0pt
...
\end{equation}
with the signal variance $s^2$, length-scale $\ell$, and noise variance
$\sigma_n^2$ optimised by maximising log marginal likelihood at each
\texttt{fit()} call. Inputs $\mathbf{x}$ are voxel coordinates $\in \mathbb{R}^3$.
Posterior moments at query point $\mathbf{x}_*$ follow the standard formula~\citep{rasmussen2006}:
\begin{equation}
