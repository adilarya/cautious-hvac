...
spread. The PPO agent learns a zero-effort policy
(effort = $0.000$) under our chosen reward shape, a diagnosed
reward-shaping failure mode rather than a fundamental RL limitation. A
controlled ablation over the cautious-term weight $\lambda \in \{0, 0.1,
0.3, 1.0, 3.0\}$ shows that $\lambda$ has no measurable effect on
aggregate ECE in our effort-budgeted regime (range $< 0.001$, within
per-seed noise): the MPC calibration advantage is driven by its
constrained-optimisation structure rather than the proposed cautious
term. All experiments are reproducible with a single command at five
seeds; the codebase ships 17 pytest cases including a regression test for
...
NoControl $0.045$, Proportional $0.031$, \textbf{MPC $0.021$}, PPO $0.035$ ---
a $2.1\times$ spread. From the reliability diagram (Fig.~\ref{fig:calib}),
...
The four controllers cluster tightly on RMSE (range $3.58$ to $3.70$~\textcelsius)
and uniformity (range $0.49$ to $0.56$~\textcelsius), but spread by a factor of
$2.1\times$ on ECE (range $0.021$ to $0.045$, pooled). MPC achieves the
...
PPO converges to a \emph{zero-effort} policy
($\overline{u} = 0.000 \pm 0.000$ across seeds): closing the vents
minimises the effort term immediately, while the RMSE term is dominated
by the same physics that drives NoControl. This is a textbook
diagnosed-failure-mode example of RL exploiting a reward-shaping
artefact~\citep{sutton2018}. The cause is the effort weight relative to
RMSE; lowering $\lambda_e$ from $0.1$ to $0.0$ would likely move PPO
into a more effort-spending regime, at the cost of removing the
energy-cost incentive. A clean $\lambda_e$ sweep would distinguish
``RL cannot solve this'' from ``RL is sensitive to reward shaping'';
we expect the latter. Independently, PPO achieves the \emph{best GP MAE}
($0.44$) and the lowest temperature standard deviation ($0.49$). With
vents closed, the field is dominated by slow, near-symmetric diffusion
of occupancy heat into a uniform ambient bath, which the GP fits almost
perfectly with few sensors. This is a diagnosed counter-result, not an
inherent RL weakness: under-actuation can sometimes simplify the
...
0.021 (pooled across seeds), the best of the four conditions and a
factor of $2.1\times$ better than the worst (NoControl, ECE $= 0.045$).
...
kernel can fit honestly. The PPO agent exposes a diagnosed reward-shaping
failure mode (zero effort, highest RMSE) but achieves the best GP fit
and lowest field uniformity --- because closing the vents simplifies
the inference problem at the cost of control. The codebase is
open-source, ships a 17-test pytest suite including a regression that
